package com.maxxposure.app.cropper;
...
import com.maxxposure.app.cropper.animation.SimpleValueAnimator;
import com.maxxposure.app.cropper.animation.SimpleValueAnimatorListener;
import com.maxxposure.app.cropper.animation.ValueAnimatorV14;
import com.maxxposure.app.cropper.animation.ValueAnimatorV8;
import com.maxxposure.app.cropper.callback.Callback;
import com.maxxposure.app.cropper.callback.CropCallback;
import com.maxxposure.app.cropper.callback.LoadCallback;
import com.maxxposure.app.cropper.callback.SaveCallback;
import com.maxxposure.app.cropper.util.Logger;
import com.maxxposure.app.cropper.util.Utils;
...
import com.maxxposure.app.view.ScrImageListing;
import com.maxxposure.app.view.ScrImagePreview;
...
public class CropImageView extends AppCompatImageView {
    ...
    private boolean cropOnTouch = false;
    public static final int YELLOW_TOUCH = 1;
    public static final int ORANGE_TOUCH = 2;
    public static final int RED_TOUCH = 3;
    public static int selectedTouch = -1;
    private Bitmap mBitmap = null;
    private boolean isAnyHintAdded = false;

    private ScrImagePreview activity = null;

    public void setActivity(ScrImagePreview activity) {
    ...
    public static DamagePointsData data = null;
   ...
   /* mPaintTranslucent.setAntiAlias(true);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (!cropOnTouch) {
            ...
            dX = event.getX();
            dY = event.getY();
            DamagePoint point = VehicleFormData.getInstance().getDamagePoint(dX, dY);
            if (point == null) {
                activity.showDialog(point);
            } else {
                activity.showDescription(point.getX(), point.getY(), point.getDescription());
            }

         /*   float x = event.getX();
            float y = event.getY();
            String pSize = data.getPreviewSize();
            String tokens[] = pSize.split("X");
            float w = Float.parseFloat(tokens[0]);
            float h = Float.parseFloat(tokens[1]);
            ...
            float upY = y + 25;
            float downY = y - 25;
            ArrayList<DamagePoint> points = data.getDamagepoints();
            ...
            try {
                activity.showDescription(event.getX(), event.getY(), retPoint.getDescription());
            } catch (Exception e) {
                e.printStackTrace();
            }*/
        ...
        if (!mIsCropEnabled && !cropOnTouch) return false;
    ...
    private void moveFrameOnTouch(float x, float y) {
        try {
            setCropEnabled(true);
            mFrameRect.left = x / 2;
            mFrameRect.top = y / 2;
            mFrameRect.right = (x / 2) + 300;
            mFrameRect.bottom = (y / 2) + 300;
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public boolean isCropOnTouch() {
        return cropOnTouch;
    }

    public void setCropOnTouch(boolean cropOnTouch) {
        this.cropOnTouch = cropOnTouch;
        if (cropOnTouch)
            moveFrameOnTouch(getWidth() / 2, getHeight() / 2);
    }
    ...
    public Bitmap mergeBitmap(int option, String title, String desc) {
        isAnyHintAdded = true;
        ...
        Bitmap dot = null;
        if (option == RED_TOUCH) {
            dot = BitmapFactory.decodeResource(getResources(),
                    R.drawable.ic_red);
        } else if (option == YELLOW_TOUCH) {
            dot = BitmapFactory.decodeResource(getResources(),
        ...
        Bitmap scaled = Bitmap.createScaledBitmap(dot, DOT_SIZE, DOT_SIZE, true);

        float xP = dX - (DOT_SIZE / 2f);
        float yP = dY - (DOT_SIZE / 2f);

        // draw watermark into
        cv.drawBitmap(scaled, xP, yP, alphaPaint);
        // save all clip
        cv.save();
        // store
        cv.restore();
        setImageBitmap(newb);
        FileUtils.saveBitmapToSDCard("" + ScrImageListing.selectedOption, newb);
        DamagePoint point = new DamagePoint();
        point.setX((int) dX);
        point.setY((int) dY);
        point.setDamageType(selectedTouch);
        point.setTitle(title);
        point.setDescription(desc);
        VehicleFormData.getInstance().setDamagePoints(point);
        invalidate();
        return newb;
    }
